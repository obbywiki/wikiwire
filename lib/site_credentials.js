...
exports.parse_site_credentials = parse_site_credentials;
function parse_site_credentials(raw) {
    ...
    for (const [site_id, value] of Object.entries(parsed)) {
        if (value === null || typeof value !== 'object' || Array.isArray(value)) {
            throw new Error(`WikiWire: site_credentials["${site_id}"] must be a JSON object with username and password`);
        }
        const rec = value;
        if (typeof rec.username !== 'string' || typeof rec.password !== 'string') {
            throw new Error(`WikiWire: site_credentials["${site_id}"] must have string username and password`);
        }
        const username = rec.username.trim();
        const password = rec.password.trim();
        if (!username || !password) {
            throw new Error(`WikiWire: site_credentials["${site_id}"] must have non-empty username and password`);
        }
